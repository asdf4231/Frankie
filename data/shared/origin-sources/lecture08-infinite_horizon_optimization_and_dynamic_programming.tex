...
\begin{itemize}
    ...
    \item After a suitable compact restriction, Assumptions 6.1--6.5 hold,
    so Theorems 6.1--6.6 apply and we can use the Euler equation.
    \item Technical step: For fixed $k_0 > 0$, impose $c_t,k_t \geq \epsilon > 0$
    with $\epsilon$ sufficiently small, then use concavity to verify optimality
    of the uniformly interior candidate after removing these bounds.
\end{itemize}
\end{frame}
