...
```
frankie                              # 默认进入 chat 模式
frankie chat                         # 同上，显示 REPL 界面

frankie ingest                       # 扫描全部 origin-sources/（有确认提示）
frankie ingest <PATH>                # 摄取文件或目录一层
frankie ingest <PATH> -r             # 穿透子目录递归摄取
frankie ingest <PATH> -f             # 强制重摄取（忽略 done 状态）
frankie ingest <PATH> --wide         # 加载更多 Wiki 上下文（50 页 vs 默认 10 页），适合 Wiki 已积累大量内容时使用
frankie ingest <PATH> --dry-run      # 预览待处理文件，不执行
frankie ingest -r --dry-run          # 预览全部 origin-sources/ 下待处理文件，不执行

frankie query <问题>                 # 向 Wiki 提问（基于当前 Wiki 内容）
frankie query <问题> -a              # 同上，并将答案归档为 Wiki 页面
frankie query <问题> -r              # 使用深度推理模型（deepseek-v4-pro）

frankie lint                         # Wiki 健康检查（矛盾/孤立页面/缺失链接）
frankie status                       # 显示 Vault/Wiki/LLM 状态
frankie sources                      # 列出 origin-sources 目录结构和文件状态
```
...
.frankie/            Frankie 私有状态（.gitignore 已排除，不属于 Wiki 也不属于代码）
├── ingest_log.json  文件摄取状态机（hash / status / ingested_at 等）
├── token_log.json   LLM 调用计费日志（时间、命令、model、token 数）
├── persona.json     Frankie 人格与角色扮演配置（语气、风格、偏好，待实现）
└── friendship.md    Frankie 对用户的认知摘要（关注领域、思维偏好、重要决策，定期由 Frankie 自更新，待实现）
```
...
- [x] **Schema 层独立**：创建 `src/frankie/schema.py` 作为 frontmatter 规范的单一真相来源，提供 `WIKI_PAGE_SCHEMA` 文本描述（注入 LLM prompt）和 `make_*_metadata()` 工厂函数；`type` 字段统一放置在 frontmatter 第一位，遵循 OKF（Open Knowledge Format）最佳实践
- [x] **烟雾测试（Smoke Test）**：`tests/test_smoke.py` 提供核心功能快速验证，测试 ingest 基本流程、query 查询、query 归档功能；通过 `frankie-smoke` 命令一键运行
...
- [x] **chat 归档（`/save`）**：用户手动输入 `/save [主题]`，Frankie 将当前对话整理为结构化洞见页并写入 `insights/` 子目录，frontmatter 标注 `type: insight`、`source: chat`
- [x] **DeepSeek 账户余额**：`frankie status` 命令实时查询并展示 DeepSeek 账户余额（`/user/balance` 接口），3 秒超时静默失败
- [x] **sources 命令显示文件状态**：`frankie sources` 顶层命令及 `/sources` 内联命令在目录树旁标注 new/done/changed/empty，done 和 changed 附带最后摄取日期（MM-DD）；基于 mtime vs ingested_at 对比，不重新计算 hash，零 IO 开销
- [x] **token_log.json**：每次 LLM 调用（ingest/query/lint/chat/save）后写入 `.frankie/token_log.json`，字段：`timestamp`、`command`、`model`、`prompt_tokens`、`completion_tokens`、`total_tokens`；`frankie status` 展示累计调用次数、总 token 数及按指令/模型分布
...
- [ ] **`/digest` 对话摘要**：每隔 N 轮或用户主动触发，Frankie 将整段对话提炼为结构化笔记写回 `insights/`；是 `/save` 的自动化升级版
...
#### 网络感知能力
- [ ] **Web 搜索工具**：赋予 Frankie 检索实时网络信息的能力（Brave Search API / Tavily 等），chat 和 query 时可主动查询补充知识盲区；与本地 Wiki 形成"长期记忆（Wiki）+ 实时检索（Web）"的双轨结构
- [ ] **网页摘取**：给定 URL，Frankie 自动抓取正文、生成摘要、存入 `sources/`，省去手动用 Obsidian Web Clipper 的步骤

#### 人格与情感层（Frankie 角色化）
- [ ] **`persona.json`**：Frankie 人格配置，支持语气、角色扮演风格、回答偏好等自定义；每次启动注入 system prompt
- [ ] **`friendship.md`**：Frankie 对用户的认知摘要，记录用户的关注领域、思维偏好、重要决策节点；由 Frankie 定期自主更新，每次对话启动时作为"私人记忆"注入 context
- [ ] **情绪感知**：分析用户输入的情绪倾向（疲惫、兴奋、困惑等），动态调整 Frankie 的回应风格和信息密度
- [ ] **交互式反馈**：Frankie 主动生成问题或挑战用户的观点，而非只是回答，形成真正的双向思想碰撞

#### 虚拟形象层（远期）
- [ ] **Frankie 电子形象**：基于 Frankie Necrofizzle 角色的视觉化虚拟助理，Web UI 内嵌实时交互界面
